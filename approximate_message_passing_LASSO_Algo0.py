...
sig2 = 1e-7 #Noise variance
lam = sig2 #free parameter for soft-threshold function
...
#Added noise vector (n x 1), iid Gaussian w/ mean 0, var sig2
y = np.dot(A,x0) + np.random.normal(0, np.sqrt(sig2), (n,1))
...
z = y #z(0)
gamma = 1.0/np.sqrt(n)
#gamma(0) (depending on channel parameter)
# Note: This free-parameter significantly affects the resulting performance.
x = eta(np.dot(A.T,z),gamma+lam) #x(1)= z(0),gamma(0)

#start main algo.
for i in range(itermax): #iteration count
    #calculate <\eta^{\prime} (A^* z(t-1) + x(t-1));\gamma(t-1)>
    m = d_eta(np.dot(A.T,z) + xp,gamma+lam).mean()
    #Factor update LHS: z(x(t), z(t-1), x(t-1), gamma(t-1))
    z = y - np.dot(A,x) + np.dot(np.dot(1.0/delta, z),m)
    #update: gamma(gamma(t-1), z(t-1), x(t-1))
    gamma = (gamma+lam)/delta * m
    #hold previous value before update x(t)
    xp = x
    #Variable update rule LHS: x(t+1)
    x = eta(np.dot(A.T,z) + x,gamma+lam)
